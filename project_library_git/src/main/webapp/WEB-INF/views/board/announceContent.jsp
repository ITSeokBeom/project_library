...
<div id="wrap">
	...
	<table id="notice">
		<tr>
			...
			<td class="left" width="500">${ announceVo.num }</td>
		</tr>
		<tr>
			<th scope="col" class="tread">조회수</th>
			<td class="left">${ announceVo.readcount }</td>
		</tr>
		<tr>
			<th scope="col" class="tdate">작성일자</th>
			<td class="left"><fmt:formatDate value="${ announceVo.regDate }" pattern="yyyy.MM.dd"/></td>
		</tr>
		<tr>
			<th scope="col" class="ttitle">글제목</th>
			<td class="left">${ announceVo.subject }</td>
		</tr>
		<tr>
			<th scope="col" class="ttitle" height="300px;">글내용</th>
			<td class="left">${ announceVo.content }</td>
		</tr>
	</table>
	...
	<div id="modi_btns_1">
		...
		<c:choose>
			<c:when test="${ id eq admin }">
				<p><input type="button" value="글수정" class="btn1" onclick="location.href = '/board/announce/modify?num=${ announceVo.num }&pageNum=${ pageNum }'"></p>
...
<script>
	function remove() {
		...
		location.href = '/board/announce/delete?num=${ announceVo.num }&pageNum=${ pageNum }';
	} // remove
</script>
